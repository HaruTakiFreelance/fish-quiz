...
# 写真が右側にある場合の列範囲
PHOTO_RIGHT_START = 0.50
PHOTO_RIGHT_END   = 1.00

# セクション内の縦トリム（解説テキスト行・アイコン行を除く）
# 上端マージン: セクション高の何%を除くか
SECTION_TOP_MARGIN    = 0.02
# 下端マージン: セクション高の何%（撮影地・水深・アイコン行）を除くか
SECTION_BOTTOM_MARGIN = 0.18
...
def recrop(data: list[dict]) -> list[dict]:
    ...
    for entry in data:
        ...
        # ── 解説画像: セクション行 全幅（上下に僅かな余白削り） ─────────
        margin_v     = int(sec_h * 0.01)
        section_crop = img.crop((
            0,
            max(0, sec_top + margin_v),
            w,
            min(h, sec_bot - margin_v),
        ))
